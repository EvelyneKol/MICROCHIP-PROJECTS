...
ISR(TCA0_CMP0_vect){  //interrupt οταν περνάει το τρένο
	...
	PORTD.OUT |= PIN2_bm;
	SPLIT_TCA0();   //κλήση χρονιστή για το T2 που χρειαζόμαστε για τους πεζούς όταν περνάνε τα τρένα
	train_passes = 1;
}
...
ISR(PORTF_PORT_vect){//interrupt για το όταν πατηθεί το κουμπί των πεζών
	...
	PORTF.INTFLAGS=y;
	if(train_passes ==1 || pedestrian_button == 1)// όταν το κουμπί έχει ήδη πατηθεί ή περνάει το τρένο δεν έχει καμία λειτουργία
	;
